...
//
                /*if ( ($handle0 = fopen($storage."interaction_correlations_basal-2019q1/".$line[$row]."-DepMap_pearsons_2019q1.csv", "r") ) !== FALSE) {

                    while (($line0 = fgetcsv($handle0,  0, ",")) !== FALSE){

                        if(isset($orphan_pairs[$line0[1]]) && !isset($gene_paper_greater_than_25[$line0[1]])){
                            //$orphan_pairs[$line0[1]][1] = ($orphan_pairs[$line0[1]][1] + $line0[2])/2;
                            $orphan_pairs[$line0[1]][1][] = $line0[2];
                            //$orphan_pairs[$line0[1]] = array_replace($orphan_pairs[$line0[1]], [1 => ($orphan_pairs[$line0[1]][1] + $line0[2])/2]);
                        }                  
                    }
                ...
                if ( ($handle0 = fopen($storage."interaction_correlations_basal-2018q4/".$line[$row]."-DepMap_pearsons_2018q4.csv", "r") ) !== FALSE) {
                    ...
                    while (($line0 = fgetcsv($handle0,  0, ",")) !== FALSE){
                        ...
                        if(isset($orphan_pairs[$line0[1]]) && !isset($gene_paper_greater_than_25[$line0[1]])){
                            //$orphan_pairs[$line0[1]][1] = ($orphan_pairs[$line0[1]][1]*2 + $line0[2])/3;
                            $orphan_pairs[$line0[1]][1][] = $line0[2];
                        }                  
                    }
                ...
                foreach($orphan_pairs as $k => $v){
                    $v1 = $v;
                    $v1[1] = array_sum($v[1])/count($v[1]);
                    $orphan_pairs1[$k] = $v1;
                }

                $orphan_pairs = $orphan_pairs1;*/
                ...
                foreach($orphan_pairs as $row0){
                    ...
                    if($row0[3]==0){
                        if(!isset($new_arr[$row])){
                            ...
                            $new_arr[$row] = $row0;
                            $new_arr[$row] = array_replace($new_arr[$row], [0 => $row0[0]."-top-hit-".$line[$row]] );
            ...
            foreach($new_arr as $row){
                $new_arr0[] = round($row[1], 3)."-corr-".$row[0];
                //$new_arr0[] = $row[1];
                //$new_arr0[] = $row[3];
            }
